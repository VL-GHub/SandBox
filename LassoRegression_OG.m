%% Lasso Regression (5-fold CV) on Synthetic Polynomial Data
close all;
clear;
clc;
...
for li = 1:numL
    ...
    for k = 1:K
        ...
        Xtr_fold  = X_combined(trID,:);
        ytr_fold  = y_combined(trID);

        Xval_fold = X_combined(valID,:);
        yval_fold = y_combined(valID);

        % Lasso fit (intercept NOT penalized)
            [B, FitInfo] = lasso(Xtr_fold, ytr_fold, 'Lambda', lambda, 'Standardize', false);
            beta_fold = B(:,1);
            intercept_fold = FitInfo.Intercept(1);

        % Predictions on validation fold
            ypred = Xval_fold * beta_fold + intercept_fold;
...
%% 7. Select best lambda and refit on full training + validation set
% Select best lambda
    [bestErr, bestIdx] = min(cvError);
    bestLambda = lambdas(bestIdx);

[B_best, FitInfo_best] = lasso(X_combined, y_combined, 'Lambda', bestLambda, 'Standardize', false);
beta_final = B_best(:,1);
...
title('Lasso Regression CV Curve');
